...
import { Building2, Mail, Lock, ShieldCheck, ExternalLink, AlertTriangle, CheckCircle2 } from "lucide-react";
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">
            Rejestracja dla{" "}
            <span className="text-blue-400">{invite.company_name}</span>
          </h1>
          <p className="mt-2 text-sm text-gray-400">
            Twój panel firmowy jest gotowy — wystarczy ustawić hasło i potwierdzić e-mail.
          </p>
        </div>
        ...
        <div className="rounded-xl bg-gray-900 border border-gray-800 p-5 mb-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-gray-500 uppercase tracking-wide mb-0.5">Firma</p>
              <p className="text-base font-semibold text-white truncate">{invite.company_name}</p>
              <p className="text-xs text-gray-400">{companyTypeLabel}</p>
            </div>
            <div className="shrink-0 px-2.5 py-1 rounded-full bg-green-500/10 border border-green-500/20">
              <span className="text-xs font-medium text-green-400">Gotowe</span>
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            {invite.nip && (
              <div>
                ...
                <p className="text-sm text-white font-mono">{invite.nip}</p>
              </div>
            )}
            {invite.krs && (
              <div>
                <p className="text-xs text-gray-500">KRS</p>
                <p className="text-sm text-white font-mono">{invite.krs}</p>
              </div>
            )}
            {invite.regon && (
              <div>
                <p className="text-xs text-gray-500">REGON</p>
                <p className="text-sm text-white font-mono">{invite.regon}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Form */}
        <div className="rounded-2xl bg-gray-900 border border-gray-800 p-6 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-gray-800/50 border border-gray-700/50">
                ...
                <span className="text-sm text-gray-300">{invite.recipient_email}</span>
                <ShieldCheck className="h-4 w-4 text-blue-400 ml-auto shrink-0" />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  required
                  className="w-full pl-11 pr-4 py-3 rounded-xl bg-gray-800 border border-gray-700 text-white text-sm placeholder:text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                />
              </div>
            </div>
            ...
            {/* Password repeat */}
            <div>
              <label htmlFor="password-repeat" className="block text-xs font-medium text-gray-400 mb-1.5">
                Powtórz hasło
              </label>
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-500" />
            ...
            <label className="flex items-start gap-3 cursor-pointer group">
              <div className="relative mt-0.5 shrink-0">
                ...
                />
                <div className="w-5 h-5 rounded border border-gray-600 bg-gray-800 peer-checked:bg-blue-600 peer-checked:border-blue-600 flex items-center justify-center transition-all">
                  {authorized && <CheckCircle2 className="h-3.5 w-3.5 text-white" />}
                </div>
              </div>
              <span className="text-xs text-gray-400 leading-relaxed">
                Potwierdzam, że jestem osobą uprawnioną do zarządzania firmą{" "}
                <span className="text-white">{invite.company_name}</span> lub działam za zgodą osoby
                uprawnionej.
              </span>
            </label>
